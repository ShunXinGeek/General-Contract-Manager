...
- **话题侧边栏**：管理助手默认展开“话题”侧边栏；可新建、搜索、重命名、置顶、拖拽排序、归档或切换话题，分别恢复每个话题的完整聊天记录与草稿
- **档案库与跨设备同步**：归档话题可恢复或永久删除；话题元数据和逐条消息按当前登录用户同步至 Firebase，离线修改会先保存在本地并在网络恢复后补传
- **消息操作**：支持复制消息、重新生成回复、删除单条消息，并可从任意大模型回复创建独立编号的话题分支继续沟通
- **附件分析**：可一次添加 TXT、Markdown、PDF、DOCX 附件；正文在浏览器本地解析，原始字节不上传。消息仅保存文件摘要，附件正文单独保存并在需要时发送给聊天模型；知识库检索不会读取附件全文
...
- **精确与联合检索**：明确合同身份和编号时精确取回；综合问题合并当前正文关键词与有效向量候选，按排名融合，可用时对身份、标题和相关正文进行重排，不因一次交叉引用命中而停止综合检索
- **有依据的关系补齐**：静态 GCC↔SCC 关系须匹配已知原文指纹；未知版本仅从当前原文明确修改语句建立关系。补齐修改与一层明确引用，超预算时明确说明证据缺口
- **索引与答案校验**：旧向量及正文、标题、模型、服务空间或维度不匹配记录不参加语义检索，保留数据并提示主动更新。回答检查引用身份、直接引文及已提供修改的引用，最多修正一次；不替代语义与适用性审查
...
2. 构建消息列表 (`buildMessagesForAPI`):
   - 如开启知识库模式 → 等待知识库初始化 → 恢复有效用户会话中的指代 → 明确编号精确取回，综合问题执行关键词/有效向量融合与可选正文重排 → 补齐可信修改及有限引用 → 按预算提供完整条款与缺口说明
   - 系统规则区分原文、解释、用户事实、条件性适用判断与待核事项；历史助手回答不是证据
   - 思考开关由既有服务商能力适配传递原生参数，继续处理 `reasoning_content`，不以思考文字长度衡量正确性
3. 发起 SSE 流式请求 → 实时解析 `data:` 行 → 分流处理 `reasoning_content` 和 `content`
4. 渲染 Markdown (marked.js) → 自动将条款引用转换为可点击超链接
5. 知识库回答完成后检查引用身份、直接引文与已提供修改引用；有问题最多进行一次同模型定向修正，仍失败则显示具体提示。保存聊天记录、校验及局部检索诊断到 IndexedDB

检索优化的实测范围、索引兼容和未完成验收项见 [管理助手检索验证报告](docs/assistant-retrieval-verification.md)。
...
2. 左上角“切换侧边栏”可收起或展开话题列表；点击 `＋` 新建话题，点击话题即可恢复其对话，拖拽同一分组的话题可调整顺序
3. 每个话题的 `⋯` 菜单支持重命名、置顶、上移/下移和归档；底部“档案库”可恢复或永久删除已归档话题
4. 使用"引用条款"按钮可视化选择条款全文并插入输入框
5. 切换"思考模式"查看 AI 推理过程
6. 切换"知识库模式"启用 RAG 语义搜索 + 交叉引用索引
7. 点击 AI 回复中的条款链接直接跳转到对应原文
